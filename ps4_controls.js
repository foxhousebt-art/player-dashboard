...
function open(name){
 if(name==="designs"){
   let body=$("systemPanelBody");
   body.innerHTML=`<h2>// DESIGNS</h2><div id="worldPickerMount"></div><p id="panelMsg"></p>`;
   if(window.PlayerWorlds)PlayerWorlds.mountPicker($("worldPickerMount"));
   else $("worldPickerMount").innerHTML="<p>Le gestionnaire de designs n’a pas pu être chargé.</p>";
   $("systemPanel").classList.add("open");$("systemPanel").setAttribute("aria-hidden","false");return;
 }
 if(!S)return;
 let body=$("systemPanelBody"),g=S.goals||{},f=S.finance||{};
...
document.querySelectorAll("[data-panel]").forEach(b=>b.onclick=()=>open(b.dataset.panel));
$("closePanel").onclick=()=>{$("systemPanel").classList.remove("open");$("systemPanel").setAttribute("aria-hidden","true")};
$("systemPanel").onclick=e=>{if(e.target===$("systemPanel"))$("closePanel").click()};
